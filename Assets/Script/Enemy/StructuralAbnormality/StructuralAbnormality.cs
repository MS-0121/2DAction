...
public class StructualAbnormality : MonoBehaviour, EnemyInterface
{
    [Header("キャラ詳細")]
    [Header("名前")]
    ...
    /*void OnCollisionStay2D(Collision2D other){
        var Target = other.gameObject.GetComponent<PlayerInterface>();
        if(Target != null)
    }*/

    //ダメージを与える側が受ける側にインターフェースがあるかを確認
    //ある場合は受ける側のダメージ処理関数を与える側で実行（与える側に設定されたATKを送る)
}
